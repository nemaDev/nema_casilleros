...
Padlock.Close = function() {
    ...
    $.post('https://nema_casilleros/PadLockClose');
}
...
Keypad.Open = function(data) {
    ...
    $( "#keypad" ).html(
        "<form action='' method='' name='PINform' id='PINform' autocomplete='off' draggable='true'>" +
            ...
            "<br>" +
            "<input type='button' class='PINbutton clear' name='-' value='borrar' id='-' onClick=clearForm(this); />" +
...
Keypad.Close = function(data) {
    ...
    $.post('https://nema_casilleros/PadLockClose');
    if (data.error != null) {
        $.post('https://nema_casilleros/CombinationFail');
    }
}
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post("https://nema_casilleros/UseCombination", JSON.stringify({
        type: type,
        combination: e.value,
    }));
};
...
findCombo = function(comboArr){
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        ...
        if (angle > (comboArr[i] - numOffset) * tickAngle &&
            ...
            // on unlock
            $.post('https://nema_casilleros/callcops');
            if (i == comboArr.length - 1) {
                // unlock :)
                $.post('https://nema_casilleros/PadLockSuccess');
                Padlock.Close();
            }
        }
        break;
...
function outOfPins() {
    gameOver = true;
    $.post('https://nema_casilleros/fail');
    setTimeout(function(){
        reset()
    }, 250)
}
...
function unlock() {
    gameOver = true;
    $.post('https://nema_casilleros/success');
    solveDeg = (Math.random() * 180) - 90
    solvePadding = 4
    maxDistFromSolve = 45
    pinRot = 1
